...
- `users` 1:N `study_groups` — 한 사용자가 여러 스터디를 생성할 수 있다.
  `study_groups.creator_id`가 `users.user_id`를 참조한다.

- `users` N:M `study_groups` — 참여 관점에서 다대다 관계이다.
  `study_members` 중간 테이블로 분리했다.

## 무결성 제약

- `users.email` UNIQUE — 이메일 중복 방지
- `study_groups.max_members` CHECK (2~100) — 비현실적인 정원 방지
- `study_members (group_id, user_id)` 복합 PK — 동일 사용자 중복 참여 방지
- `study_groups.creator_id` ON DELETE RESTRICT — 참여자가 있는 스터디 생성자 삭제 방지
- `study_members.group_id/user_id` ON DELETE CASCADE — 스터디·사용자 삭제 시 참여 기록 자동 정리
...
Serializable 충돌(`40001`) 발생 시 409로 응답한다.
